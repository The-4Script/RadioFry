"""Train the explainable interleaver classifier from a manifest."""
...
def train_interleaver(manifest_path: str | Path, output_path: str | Path, *, seed: int = 7) -> dict[str, object]:
    ...
    labels = np.array([row["interleaver_type"] for row in rows])
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train the RadioFry interleaver classifier")
    parser.add_argument("--manifest", default="data/synthetic/interleaver/manifest.csv")
    parser.add_argument("--output", default="models_saved/interleaver_classifier.pkl")
    parser.add_argument("--seed", type=int, default=7)
    args = parser.parse_args()
    print(json.dumps(train_interleaver(args.manifest, args.output, seed=args.seed), indent=2))
