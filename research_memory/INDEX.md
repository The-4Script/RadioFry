...
- **Entry 047**: RadioML 2018.01A (Dataset 2) - **first real-data trained model**. Frozen V3 scores 15.97% zero-training; **finetune reaches 44.46%** on the same sealed frames (2.8x), 42.62% on the full 24-class task and **67.06% at >= +10 dB**. linear_probe 30.19% => V3's features transfer PARTIALLY but are not directly reusable; finetune beats scratch by only 6.0 pts => synthetic pre-training is a modest help, not a head start. **Zero confident-wrong predictions** (wrong-answer median confidence 0.058, max 0.588). **DeepSig's shipped classes.txt ordering is WRONG** - impropriety agrees 24/24 with classes-fixed, 10/24 with shipped. Leakage: near-duplicates **0.0%** (vs Dataset 1's 48% for QAM). GPU sm_120 verified, 25x CPU. Candidate NOT promoted
