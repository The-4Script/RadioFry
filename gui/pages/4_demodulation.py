...
if report is None:
    ...
    render_method_note("Method and limits", "Demodulation uses the selected modulation family and a coarse search across integer sample offsets within one estimated symbol. Analog AM/FM paths are currently decimated with the same baseline timing path, which can discard audio detail; fractional timing recovery, carrier recovery, and protocol-specific framing may still need expert calibration.")
